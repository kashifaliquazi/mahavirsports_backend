...
adminModel.getusers = async (body) =>{

    try {
        let mysql = await getConnection();
        console.log("got connection")
        let query =`select * from mahavirsports.users WHERE role not in('ADMIN')`;
        if(body.userid || body.role ){
            //query += 'where '
            if(body.userid){
                query += `${getMysqlQueryPhrase(query)} userid = ${body.userid} `
            }
            if(body.role){
                query += `${getMysqlQueryPhrase(query)} role in('${body.role}') `
            }
        }   
...
adminModel.addServiceBoy = async (body) =>{

    try {
        console.log("getting user data",body.mobileno,body.password)
        let mysql = await getConnection();
        console.log("got connection")
        let query =`insert into mahavirsports.users(name,mobileno,password,role,status) values('${body.name}','${body.mobileno}','${body.password}','SERVICEUSER','ACTIVE');`;
        console.log("query ", query);
        let results = await mysql.query(query);
        await mysql.end();
        console.log("results ", results);
...
adminModel.getPurchases = async (body) =>{
    ...
    try {
        ...
        if(body.userid || body.purchaseid || body.initiatedby){
            ...
            if(body.purchaseid){
                query += `${getMysqlQueryPhrase(query)} p.purchaseid = '${body.purchaseid}' `
            }
            if(body.initiatedby){
                query += `${getMysqlQueryPhrase(query)} p.initiatedby = '${body.initiatedby}' `
            }
        }   
